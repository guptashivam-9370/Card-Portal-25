...
const OTPComponent = () => {
	...
	const sendOTP = async (email) => {
		setSending(true);
		try {
			const response = await fetch(`${API_BASE_URL}/api/generate-otp-email/`, {
				method: 'POST',
				headers: {
	...
	const verifyOTP = async (email, otp) => {
		try {
			const response = await fetch(`${API_BASE_URL}/api/verify-otp-email/`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
				},
	...
	const handleVerifyOTP = async () => {
		...
		try {
			...
			router.push(`/download?user_id=${userId}`);
		} catch (err) {
			setcorrectOTP(false);
		...
		<div className='home-page'>
			...
			<div className='chat'>
				...
				<div className='mobile-break'>
					{otpSent ? (
						<>
							<img className='tick' src='images/success.svg' />
							<p
								className='white'
